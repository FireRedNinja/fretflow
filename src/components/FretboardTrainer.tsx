  ...
  const getRandomNotePosition = useCallback((): NotePosition | null => {
    ...
    while (attempts < 100) {
      ...
      const randomVisualStringIndex =
        availableVisualStrings[
          getRandomInt(0, availableVisualStrings.length - 1)
        ];
      const randomFret = getRandomInt(
        settings.fretRange.min,
        settings.fretRange.max
      );

      const logicalStringIndex =
        STANDARD_TUNING.length - 1 - randomVisualStringIndex;
    ...
    console.error(
      "Could not find a valid note position within the current settings after 100 attempts."
    );
    setSettingsError(
      "Could not generate a question with the current settings. Try broadening the range or note types."
    );
    return null;
      ...
      setHighlightedNotes([
        {
          ...currentQuestion.position,
          color: "bg-green-500",
          label: currentQuestion.correctNote,
        },
      ]);
    } else {
  ...
  const handleFretClick = (clickedPosition: NotePosition) => {
    ...
    if (
      ...
      setHighlightedNotes((prev) => [
        ...prev,
        { ...clickedPosition, colour: "bg-gray-400", label: "X" },
      ]);
    ...
    if (isCorrect) {
      ...
      setHighlightedNotes([
        { ...clickedPosition, color: "bg-green-500", label: clickedNoteName },
      ]);
    } else {
      setFeedback(
        `Incorrect. That was ${clickedNoteName}. Try finding ${currentQuestion.targetNote}.`
      );
      // Highlight the incorrectly clicked note
      setHighlightedNotes([
        { ...clickedPosition, color: "bg-red-500", label: clickedNoteName },
      ]);
        ...
        <div className="p-4 border-t border-gray-300 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center space-x-3">
              <input
                type="number"
                name="min"
                value={tempSettings.fretRange.min}
                onChange={handleFretRangeChange}
                min="0"
                max={maxFrets}
                className="w-16 p-1 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500"
              ...
              <input
                type="number"
                name="max"
                value={tempSettings.fretRange.max}
                onChange={handleFretRangeChange}
                min="0"
                max={maxFrets}
                className="w-16 p-1 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500"
            ...
            <div className="flex items-center space-x-4">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  name="noteType"
                  value="all"
                  checked={tempSettings.noteType === "all"}
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  name="noteType"
                  value="natural"
                  checked={tempSettings.noteType === "natural"}
                  onChange={handleNoteTypeChange}
                  className="text-blue-600 focus:ring-blue-500"
                />
          ...
          <div className="pt-3 text-right">
            ...
            <button
              ...
              className="py-2 px-5 bg-green-600 text-white rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-400 transition duration-150 ease-in-out shadow"
            >
              ...
              (quizState === "question" || quizState === "answered") && (
                <>
                  ...
                  <Fretboard
                    ...
                    fretCount={
                      settings.fretRange.max > 12 ? settings.fretRange.max : 12
                    }
                    tuning={visualTuning}
                    showStringLabels={quizState !== "question"}
